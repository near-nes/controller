...
def calculate_rmse(
    ...
    """Calculate the RMSE of the elbow throughout the trials"""

    single_trial_steps = len(config.time_vector_single_trial_s)
    desired_single_trial = desired_trj_joint_rad

    elbow_rmse = np.zeros(config.N_TRIALS)

    full_desired_plot = np.full_like(pos_j_rad_actual[:], np.nan)
    for trial_n in range(config.N_TRIALS):
        start_idx = trial_n * single_trial_steps
        end_idx = start_idx + len(desired_single_trial)
        elbow_rmse[trial_n] = np.sqrt(
            np.mean(
                pos_j_rad_actual[start_idx:end_idx]
                - desired_single_trial[start_idx:end_idx]
            )
        )
...
def plot_rmse(
    ...
    elbow_rmse = calculate_rmse(config, pos_j_rad_actual, desired_trj_joint_rad)

    plt.figure()
    plt.plot(np.arange(1, config.N_TRIALS + 1), elbow_rmse, "-o")
    plt.xlabel("Trial Number")
    plt.ylabel("Elbow RMSE (rad)")
    plt.title("Elbow RMSE Over Trials")
    plt.tight_layout()
    if save_fig:
